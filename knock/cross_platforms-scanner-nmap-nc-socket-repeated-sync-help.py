...
def tool_exists(t): return shutil.which(t) is not None
...
def main():
    parser = argparse.ArgumentParser(description="Cross-platform adaptive port scanner")
    parser.add_argument("-P", choices=["tcp", "udp"], required=True)
    parser.add_argument("-p", required=True, help="Port or range (1-1024)")
    ...
    while True:
        print("\n[*] Running scan...")
        current = scan_with_nmap(args.P, ports, args.t) if tool_exists("nmap") \
