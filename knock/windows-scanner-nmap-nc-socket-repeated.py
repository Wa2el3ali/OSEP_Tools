...
def scan_with_nmap(protocol, ports, target):
    ...
    if protocol == "tcp":
        if is_admin():
            ...
            print("[!] Not running as Administrator — falling back to TCP connect scan")
            cmd += ["-sT"]
    else:
        cmd += ["-sU"]
...
def scan_with_nc(protocol, ports, targets):
    ...
    for target in targets:
        for port in tqdm(ports, desc=f"Scanning {target}", unit="port"):
            if protocol == "tcp":
                cmd = ["ncat", "-z", "-w", "1", target, str(port)]
            else:
                cmd = ["ncat", "-u", "-z", "-w", "1", target, str(port)]
...
def main():
    parser = argparse.ArgumentParser(
        description="Adaptive Windows port scanner with change detection"
    )
